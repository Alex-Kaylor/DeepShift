...
def convert_to_shift(model, shift_depth, shift_type, shift_base, convert_all_linear=True, convert_weights=False, freeze_sign = False, use_kernel=False, use_cuda=True, rounding='deterministic', weight_bits=5):
    ...
    for name, module in reversed(model._modules.items()):
        if len(list(module.children())) > 0:
            # recurse
            model._modules[name], num_converted = convert_to_shift(model=module, shift_depth=shift_depth-conversion_count, shift_type=shift_type, shift_base=shift_base, convert_all_linear=convert_all_linear, convert_weights=convert_weights, freeze_sign = freeze_sign, use_kernel=use_kernel, use_cuda = use_cuda, rounding = rounding, weight_bits = weight_bits)
            conversion_count += num_converted
        if type(module) == nn.Linear and (convert_all_linear == True or conversion_count < shift_depth):
            linear = module
        
            if shift_type == 'Q':
                shift_linear = deepshift.modules_q.LinearShiftQ(module.in_features, module.out_features, shift_base, module.bias is not None, use_kernel=use_kernel, use_cuda=use_cuda, rounding=rounding, weight_bits=weight_bits) 
                ...
                if use_cuda==True and use_kernel == True:
                    shift_linear.conc_weight = utils.compress_bits(*utils.get_shift_and_sign(linear.weight, shift_base))
            elif shift_type == 'PS':
                shift_linear = deepshift.modules.LinearShift(module.in_features, module.out_features, shift_base, module.bias is not None, freeze_sign = freeze_sign, use_kernel=use_kernel, use_cuda=use_cuda, rounding=rounding, weight_bits=weight_bits)

                if convert_weights == True:
                    shift_linear.shift.data, shift_linear.sign.data = utils.get_shift_and_sign(linear.weight, shift_base)
        ...
        if type(module) == nn.Conv2d and conversion_count < shift_depth:
            ...
            if shift_type == 'Q':
                shift_conv2d = deepshift.modules_q.Conv2dShiftQ(module.in_channels, module.out_channels, shift_base, module.kernel_size, module.stride,
                ...
                shift_conv2d = deepshift.modules.Conv2dShift(module.in_channels, module.out_channels, shift_base, module.kernel_size, module.stride,
                                                module.padding, module.dilation, module.groups,
                                                module.bias is not None, module.padding_mode,
                ...
                if convert_weights == True:
                    shift_conv2d.shift.data, shift_conv2d.sign.data = utils.get_shift_and_sign(conv2d.weight, shift_base)
...
def round_shift_weights(model, shift_base, clone=False):
    ...
    for name, module in reversed(model._modules.items()):
        if len(list(module.children())) > 0:
            # recurse
            model._modules[name] = round_shift_weights(model=module, shift_base=shift_base)
        ...
        if type(module) == deepshift.modules.LinearShift or type(module) == deepshift.modules.Conv2dShift:
            ...
            module.weight.data = utils.round_power_of_2(module.weight, shift_base)
